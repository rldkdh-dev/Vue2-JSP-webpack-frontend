				...
				<tbody>
					<tr v-for="item in viewModel.list" :key="item">
						...
						<td>
							...
							<button v-else-if="item.file_extn == 'pdf' || item.file_extn == 'PDF'" @click="fnPdfView(item.strg_path+item.strg_file_nm,item.file_extn, item.orgnl_file_nm)" class="btn btn-small btn-white">{{ item.file_extn }}</button>
...
import PDFviewer from './pdfViewerCp.vue';
...
export default {
	...
	methods: {
		...
		fnPdfView:function(path,type, nm){
			...
			this.$nextTick(function () {
                if (this.bool) {
					this.$refs.PDFviewer.fnSetContents(path, nm,this.popupTitle);
				}
            });
