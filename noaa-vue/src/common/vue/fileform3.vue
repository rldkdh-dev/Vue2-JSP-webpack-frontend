    ...
    methods: {
        ...
        fnDropInputTag: function(e) {
            ...
            for(let i=0; i<files.length; i++){
                ...
                if(i != 0){
                    vt.viewModel.filelist[vm.no].splice((Number(targetIndex) + Number(i)), 0, {})
                }
                ...
                if(file != undefined) {
                    ...
                    if(file.size > maxSize){
                        ...
                        $('#'+this.pass.id+this.pass.no+((Number(targetIndex) + i))+' #filebox').text(fileName);
                        
                        vm.filelist[vt.pass.no][(Number(targetIndex) + i)].file = file;
                        vm.filelist[vt.pass.no][(Number(targetIndex) + i)].orgnl_file_nm = fileName;
                        vm.filelist[vt.pass.no][(Number(targetIndex) + i)].file_title = fileName.substr(0, fileName.lastIndexOf('.'));
