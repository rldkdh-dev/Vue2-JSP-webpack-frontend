                    ...
                    <tbody>
                        <template v-if="viewModel.list.length != 0">
                            <tr v-if="viewModel.list.length != 0" v-for="(item, i) in viewModel.list" :key="i">
                                ...
                                <td>{{item.dt}}</td>
                                <td class="text-left"><router-link class="btn-link" :to="'/detail/' + item.job_actpln_sn">{{item.ttl}}</router-link></td>
                                <td>{{item.pic}}</td>
                                <td><button v-if="item.STRG_FILE_NM != '' && item.STRG_FILE_NM != null && item.STRG_FILE_NM != undefined " class="btn btn-icon" @click="fnPopup(item.job_actpln_sn, 8);"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-attachment"></use></svg></button></td>
                                <td v-if="item.reg_id == viewModel.userInfo.user_id">
                                    <router-link :to="{name: 'rgstWkPlDetail', params: {job_actpln_sn:item.job_actpln_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-content-search"></use></svg></router-link>
                                    <router-link :to="{name: 'rgstWkPlUpdate', params: {job_actpln_sn:item.job_actpln_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-edit-contents"></use></svg></router-link>
                                    <a href="#" class="btn btn-icon" @click="fnDelete(item.job_actpln_sn)"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-delete-sweep"></use></svg></a>
                                </td>
                                <td v-else>
                                    <router-link :to="{name: 'rgstWkPlDetail', params: {job_actpln_sn:item.job_actpln_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-content-search"></use></svg></router-link> 
                                </td>
        ...
        export default {
            ...
            methods: {
                ...
                fnList:function(paramPageNo){
                    ...
                    let send={
                        mapperId:'rgstWkPl.selectRgstWkPlList',
                        totalId:'rgstWkPl.selectRgstWkPlListTotal',
                    ...
                    Axios.post("/selectRgstWkPlList.do",send).then(function(res){
                ...
                fnDelete:function(job_actpln_sn){
                    let vm = this;
                    let send = {
                        mapperId : 'rgstWkPl.deleteRgstWkPl',
                        job_actpln_sn : job_actpln_sn 
                    }
                    if(confirm('정말 삭제하시겠습니까?')) { 
                        Axios.post('/deleteRgstWkPl.do', send).then(function(res){
                            alert('삭제되었습니다.');
                            vm.fnList(0);                    
                        })
                    } else { 
                ...
                fnPopup: function(sn, se) {
                    ...
                    Axios.post("/selectRgstWkPlFileList.do", send)
