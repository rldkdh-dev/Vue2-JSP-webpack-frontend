            ...
            <table class="table horizontal write">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input type="text" id="pic" v-model="viewModel.data.pic" disabled></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td colspan="3"><input type="text" id="ttl" v-model="viewModel.data.ttl"></td>
                    </tr>
                    <tr>
...
<script>
    ...
    mounted: function() {
		...
		vm.fnInit();
        if(vm.acdnt_strt_dclr_sn != undefined && vm.acdnt_strt_dclr_sn != null && vm.acdnt_strt_dclr_sn != '') {
        ...
        fnInit: function() {
            ...
            fngetFileInfo:function(frm){
                ...
                let data = {
                    ...
                    acdnt_strt_dclr_sn:vm.$route.params.acdnt_strt_dclr_sn,
                    deletefileList : vm.viewModel.deletefileList,
                }

                frm.append('data', new Blob([ JSON.stringify(data) ], {type : "application/json"}));
                    
                frm.append("filePath", "rgstBsnsStrtRgst");

                if(vm.acdnt_strt_dclr_sn != undefined && vm.acdnt_strt_dclr_sn != null && vm.acdnt_strt_dclr_sn != ''){
                    if(confirm('수정하시겠습니까?')){
                        Axios.post("/updateRgstBsnsStrtRgst.do",frm).then(function(rs){
                            vm.isSaved = true;
                            vm.$router.push('/');
                            alert('수정되었습니다.');
                        })
                    ...
                    if(confirm('등록하시겠습니까?')){
                        ...
                        Axios.post("/insertRgstBsnsStrtRgst.do",frm).then(function(rs){
                            vm.isSaved = true;
                            vm.$router.push('/');
                            alert('등록되었습니다.');
                            })
