                ...
                <tbody>
                    <template v-if="viewModel.list.length != 0">
                        <tr  v-for="(item, i) in viewModel.list" :key="i">
                            ...
                            <td>{{item.dt}}</td>
                            <td class="text-left"><router-link class="btn-link" :to="'/detail/' + item.acdnt_strt_dclr_sn">{{item.ttl}}</router-link></td>
                            <td>{{item.pic}}</td>
                            <!-- <td></td> -->
                            <!-- <td v-if="item.STRG_FILE_NM != '' && item.STRG_FILE_NM != null && item.STRG_FILE_NM != undefined && item.FILE_EXTN == 'png' || item.FILE_EXTN == 'jpg' || item.FILE_EXTN == 'jpeg' || item.FILE_EXTN == 'gif' " style="width: 15%;"><img :src="'/uploadfiles/temp/prttMngmt/' + item.STRG_FILE_NM"/></td>
                             <td v-else></td> -->
                            <td><button v-if="item.STRG_FILE_NM != '' && item.STRG_FILE_NM != null && item.STRG_FILE_NM != undefined " class="btn btn-icon" @click="fnPopup(item.acdnt_strt_dclr_sn, 4);"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-attachment"></use></svg></button></td>
                            <td v-if="item.reg_id == viewModel.userInfo.user_id || (viewModel.userInfo.user_roles == 'ROLE_001' && viewModel.userInfo.user_type_code == 'ACC_0000')">
                                <router-link :to="{name: 'rgstBsnsStrtRgstDetail', params: {acdnt_strt_dclr_sn:item.acdnt_strt_dclr_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-content-search"></use></svg></router-link>
                                <router-link :to="{name: 'rgstBsnsStrtRgstUpdate', params: {acdnt_strt_dclr_sn:item.acdnt_strt_dclr_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-edit-contents"></use></svg></router-link>
                                <a href="#" class="btn btn-icon" @click="fnDelete(item.acdnt_strt_dclr_sn)"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-delete-sweep"></use></svg></a>
                            </td>
                            <td v-else>
                                <router-link :to="{name: 'rgstBsnsStrtRgstDetail', params: {acdnt_strt_dclr_sn:item.acdnt_strt_dclr_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-content-search"></use></svg></router-link>
                            </td>
    ...
    export default {
        ...
        methods: {
            ...
            fnList:function(paramPageNo){
                ...
                let send={
                    mapperId:'rgstBsnsStrtRgst.selectRgstBsnsStrtRgstList',
                    totalId:'rgstBsnsStrtRgst.selectRgstBsnsStrtRgstListTotal',
                ...
                Axios.post("/selectRgstBsnsStrtRgstList.do",send).then(function(res){
            ...
            fnDelete:function(acdnt_strt_dclr_sn){
                let vm = this;
                let send = {
                    mapperId : 'rgstBsnsStrtRgst.deleteRgstBsnsStrtRgst',
                    acdnt_strt_dclr_sn : acdnt_strt_dclr_sn 
                }
                if(confirm('정말 삭제하시겠습니까?')) { 
                    Axios.post('/deleteRgstBsnsStrtRgst.do', send).then(function(res){
                        alert('삭제되었습니다.');
                        vm.fnList(0);                    
                    })
                } else { 
            ...
            fnPopup: function(sn, se) {
                ...
                Axios.post("/selectRgstBsnsStrtRgstFileList.do", send)
