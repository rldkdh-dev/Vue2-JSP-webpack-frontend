...
<script>
    ...
    methods: {
        fnInit: function() {
            ...
            let sn = vm.$route.params.actpln_sn;
            ...
            fngetFileInfo:function(frm){
                ...
                let data = {
                    ...
                    actpln_sn:vm.$route.params.actpln_sn,
                    deletefileList : vm.viewModel.deletefileList,
                }

                frm.append('data', new Blob([ JSON.stringify(data) ], {type : "application/json"}));
                    
                frm.append("filePath", "rgstRgstDgPrvtPl");

                if(vm.actpln_sn != undefined && vm.actpln_sn != null && vm.actpln_sn != ''){
                    if(confirm('수정하시겠습니까?')){
                        vm.isSaved = true;
                        Axios.post("/updateRgstDgPrvtPl.do",frm).then(function(rs){
                            vm.$router.push('/');
                            alert('수정되었습니다.');
                        })
                    ...
                    if(confirm('등록하시겠습니까?')){
                        if(vm.viewModel.data.bplc_sn == '' || vm.viewModel.data.bplc_sn == null){
                            ...
                            Axios.post("/insertRgstDgPrvtPl.do",frm).then(function(rs){
                                vm.isSaved = true;
                                vm.$router.push('/');
                                alert('등록되었습니다.');
                            })
