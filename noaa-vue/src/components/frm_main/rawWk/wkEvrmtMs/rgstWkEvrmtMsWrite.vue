                    ...
                    <tr>
                        ...
                        <td>
                            <div class="input_group">
                                ...
                                <span>*해당 서류 승인일자를 입력해주세요.</span>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row" class="essential"><label for="cn">내용</label></th>
                        <td><textarea type="text" id="cn" v-model="viewModel.data.cn"></textarea></td>
                    </tr>
...
<script>
        ...
        mounted: function() {
            let vm = this;
            vm.msrmt_sn = vm.$route.params.msrmt_sn;
            vm.fnInit();
        },
        activated:function(){
            this.fnInit();
        ...
        methods: {
            fnInit: function() {
                ...
                let sn = vm.$route.params.msrmt_sn;
                ...
                fnDetail: function(msrmt_sn){
                    let vm = this;
                    let send = {
                        msrmt_sn: msrmt_sn,
                        mng_sn:msrmt_sn
                    };
                        Axios.post("/detailRgstWkEvrmtMsList.do",send).then(function(rs) {
                ...
                fngetFileInfo:function(frm){
                    ...
                    let data = {
                        ...
                        msrmt_sn:vm.$route.params.msrmt_sn,
                        deletefileList : vm.viewModel.deletefileList,
                    }
    
                    frm.append('data', new Blob([ JSON.stringify(data) ], {type : "application/json"}));
                        
                    frm.append("filePath", "rgstWkEvrmtMs");
    
                    if(vm.msrmt_sn != undefined && vm.msrmt_sn != null && vm.msrmt_sn != ''){
                        if(confirm('수정하시겠습니까?')){
                            Axios.post("/updateRgstWkEvrmtMs.do",frm).then(function(rs){
                                vm.isSaved = true;
                                vm.$router.push('/');
                                alert('수정되었습니다.');
                            })
                        ...
                        if(confirm('등록하시겠습니까?')){
                            ...
                            return false;
                                ...
                                Axios.post("/insertRgstWkEvrmtMs.do",frm).then(function(rs){
                                    vm.isSaved = true;
                                    vm.$router.push('/');
                                    alert('등록되었습니다.');
                                })
