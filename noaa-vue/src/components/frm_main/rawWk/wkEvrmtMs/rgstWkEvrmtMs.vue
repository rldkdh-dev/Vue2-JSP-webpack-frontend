            ...
            <div>
                <div class="search_container">
                    <div class="row">
                        ...
                        <input type="text" placeholder="내용을 입력하세요." title="내용 입력" v-model="viewModel.search.cn">
                    </div>
                </div>
                <div class="btn-group">
                    ...
                    <tbody>
                        <template v-if="viewModel.list.length != 0">
                            <tr v-if="viewModel.list.length != 0" v-for="(item, i) in viewModel.list" :key="i">
                                ...
                                <td>{{item.dt}}</td>
                                <td class="text-left"><router-link class="btn-link" :to="'/detail/' + item.msrmt_sn">{{item.cn}}</router-link></td>
                                ...
                                <!-- <td v-if="item.STRG_FILE_NM != '' && item.STRG_FILE_NM != null && item.STRG_FILE_NM != undefined && item.FILE_EXTN == 'png' || item.FILE_EXTN == 'jpg' || item.FILE_EXTN == 'jpeg' || item.FILE_EXTN == 'gif' " style="width: 15%;"><img :src="'/uploadfiles/temp/prttMngmt/' + item.STRG_FILE_NM"/></td>
                                 <td v-else></td> -->
                                <td><button v-if="item.STRG_FILE_NM != '' && item.STRG_FILE_NM != null && item.STRG_FILE_NM != undefined " class="btn btn-icon" @click="fnPopup(item.msrmt_sn, 7);"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-attachment"></use></svg></button></td>
                                <td v-if="item.reg_id == viewModel.userInfo.user_id">
                                    <router-link :to="{name: 'rgstWkEvrmtMsDetail', params: {msrmt_sn:item.msrmt_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-content-search"></use></svg></router-link>
                                    <router-link :to="{name: 'rgstWkEvrmtMsUpdate', params: {msrmt_sn:item.msrmt_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-edit-contents"></use></svg></router-link>
                                    <a href="#" class="btn btn-icon" @click="fnDelete(item.msrmt_sn)"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-delete-sweep"></use></svg></a>
                                </td>
                                <td v-else>
                                    <router-link :to="{name: 'rgstWkEvrmtMsDetail', params: {msrmt_sn:item.msrmt_sn}}" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-content-search"></use></svg></router-link> 
                                </td>
        ...
        export default {
            ...
            methods: {
                ...
                fnList:function(paramPageNo){
                    ...
                    let send={
                        mapperId:'rgstWkEvrmtMs.selectRgstWkEvrmtMsList',
                        totalId:'rgstWkEvrmtMs.selectRgstWkEvrmtMsListTotal',
                        pageNo:vm.viewModel.pager.pageNo,
                        pageSize:vm.viewModel.pager.rowSize,
                        pagingYN:'Y',
                        cn:vm.viewModel.search.cn,
                    ...
                    Axios.post("/selectRgstWkEvrmtMsList.do",send).then(function(res){
                ...
                fnDelete:function(msrmt_sn){
                    let vm = this;
                    let send = {
                        mapperId : 'rgstWkEvrmtMs.deleteRgstWkEvrmtMs',
                        msrmt_sn : msrmt_sn 
                    }
                    if(confirm('정말 삭제하시겠습니까?')) { 
                        Axios.post('/deleteRgstWkEvrmtMs.do', send).then(function(res){
                            alert('삭제되었습니다.');
                            vm.fnList(0);                    
                        })
                    } else { 
                ...
                fnPopup: function(sn, se) {
                    ...
                    Axios.post("/selectRgstWkEvrmtMsFileList.do", send)
