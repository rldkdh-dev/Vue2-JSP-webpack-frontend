<template>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="input_group">
                                    <div class="datepicker_box" style="width: 20%; float: left;">
                                        <input type="text" class="datepicker" id="dt" placeholder="날짜" title="날짜" onclick="$(this).prev().val('');" readonly>
                                    </div>
    ...
    <script>
        ...
        mounted: function() {
            let vm = this;
            vm.tchnlgy_cch_sn = vm.$route.params.tchnlgy_cch_sn;
            vm.fnInit();
        },
        activated:function(){
            this.fnInit();
        ...
        methods: {
            fnInit: function() {
                ...
                let sn = vm.$route.params.tchnlgy_cch_sn;
            ...
            vm.viewModel.data.bplc_nm = vm.viewModel.user.bplc_sn == 0 ? '' : vm.viewModel.user.bplc_nm;
                ...
                fnDetail: function(tchnlgy_cch_sn){
                    let vm = this;
                    let send = {
                        tchnlgy_cch_sn: tchnlgy_cch_sn,
                        mng_sn:tchnlgy_cch_sn
                    };
                        Axios.post("/detailRgstDsstPrvtTcnGdList.do",send).then(function(rs) {
                ...
                fngetFileInfo:function(frm){
                    ...
                    let data = {
                        ...
                        tchnlgy_cch_sn:vm.$route.params.tchnlgy_cch_sn,
                        deletefileList : vm.viewModel.deletefileList,
                    }
    
                    frm.append('data', new Blob([ JSON.stringify(data) ], {type : "application/json"}));
                        
                    frm.append("filePath", "rgstRgstDsstPrvtTcnGd");               
                    if(vm.tchnlgy_cch_sn != undefined && vm.tchnlgy_cch_sn != null && vm.tchnlgy_cch_sn != ''){
                    if(confirm('수정하시겠습니까?')){
                        Axios.post("/updateRgstDsstPrvtTcnGd.do",frm).then(function(rs){
                            vm.isSaved = true;
                            vm.$router.push('/');
                            alert('수정되었습니다.');
                        })
                    ...
                    if(confirm('등록하시겠습니까?')){
                        if(vm.viewModel.data.bplc_sn == '' || vm.viewModel.data.bplc_sn == null){
                            ...
                            Axios.post("/insertRgstDsstPrvtTcnGd.do",frm).then(function(rs){
                                vm.isSaved = true;
                                vm.$router.push('/');
                                alert('등록되었습니다.');
                            })
