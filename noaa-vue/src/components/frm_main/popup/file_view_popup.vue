						...
						<table class="table pop" v-show="bool == false">
							...
							<tbody>
								<tr v-for="item in viewModel.list" :key="item">
									...
									<td>
										<a @click="fnPdfView(item.file_path,item.file_type)" :class="item.file_type+'_icon'" style='cursor :pointer'>{{ item.file_type }}</a> <!-- pdf 뷰어 -->
									</td>
									<td>{{ item.file_size }}</td>
									<td>{{ item.regdt }}</td>
									<td>
										<a class="btn btn-icon btn-info2" @click="fileDown(item.file_seq)" download><span class="mi">file_download</span></a>
									</td>
								</tr>
							</tbody>
						</table>
		...
		fnSetContents: function(id){
    		...
    		Axios.post("/list", senddata).then(function(rs) {
				...
				for(var i=0; i<vm.list.length; i++){
                    if(vm.list[i].file_size < 1024 * 1024){
                        vm.list[i].file_size = parseInt(vm.list[i].file_size / 1024).toFixed(1) + "K";
                    }
                }
