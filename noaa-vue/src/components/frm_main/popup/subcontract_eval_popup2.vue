				...
				<tbody>
					<tr v-for="(item , ind) in viewModel.list" :key="item.uid">
						...
						<td v-if="item.regdt != undefined && item.base_point != 0" class="np">
							<select title="득점" :value="item.base_point" @change="fnpointsum(ind,$event.target.value)" :disabled="item.state2 != 0 ? true:false"> <!--disabled-->
								<option v-for="i in item.point" :key="i" :value="i">{{ i }}</option>
							</select>
						</td>
						<td v-else class="np">
							<select title="득점" :value="item.base_point" @change="fnpointsum(ind,$event.target.value)" :disabled="item.state2 != 0 ? true:false">
...
export default {
	...
	methods: {
		...
		fnSetContents: function(){
			...
			let senddata = {
				mapperId : 'subcontract.selectsubevallist2',
		...
		fnsubiteminsert:function(state){
			...
			if(this.viewModel.list[0].regdt != undefined){
				// for(var i=0; i<this.viewModel.list.length; i++){
					var send={
						mapperId:'subcontract.subpopupupdate2',
						sc_uid:this.viewModel.list[0].uid,
						score_uid:this.viewModel.list[0].score2_uid,
				...
				// for(var i=0; i<this.viewModel.list.length; i++){
					var send={
						mapperId:'subcontract.subpopupinsert2',
						sc_uid:this.viewModel.list[0].uid,
						score_uid:this.viewModel.list[0].score2_uid,
						// score_item_uid:this.viewModel.list[i].item_uid,
						memberuid:this.viewModel.list[0].memberuid,
		...
		fnrequestcancel:function(state){
			// 요청 취소 
			let send={
				mapperId:'subcontract.subpopupcancelupdate2',
				state:state,
				uid:this.uid,
			}
			Axios.post("/update",send)
		...
		fndone:function(state){
			//승인요청 
			let send={
				mapperId:'subcontract.subpopupdoneupdate2',
				state:state,
				uid:this.uid,
			}
			Axios.post("/update",send)
		...
		fndonecancel:function(state){
			// 승인 취소 
			let send={
				mapperId:'subcontract.subpopupdonecancelupdate2',
				state:state,
				uid:this.uid,
			}
			Axios.post("/update",send)
