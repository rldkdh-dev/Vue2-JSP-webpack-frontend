								...
								<tr>
                				    ...
                				    <td>
										<div class="datepicker_box">
											<input type="text" name="measure_date" class="datepicker" id="measure_date" value="" readonly>
										</div>
									</td>
                				</tr>
								<tr>
	...
	methods: {
		...
		fnSetContents: function(id){
    		...
    		Axios.post("/detail", senddata).then(function(rs) {
				...
				let file = vm.viewModel.list.measure_file_grp_id;
				if(file != ''){
					vm.$refs.files.fnFileLoad(file)
				}
            });
		},
		fnUpdate: function(){
    		let vm = this; 
    		let data = {
  	 	    	mapperId : 'riskEval.updateriskImprPlanpopupInfo',
				uid : vm.viewModel.list.uid,
				measure_date : $('#measure_date').val(),
				contents2 : vm.viewModel.list.contents2,
				file_grp_id : vm.viewModel.fileGroup,
  	 	    };
			console.log(vm.viewModel.fileGroup);
     		Axios.post("/update",data)
  			.then(function(response) {
				if(vm.viewModel.fileGroup!='' && vm.viewModel.fileGroup != null) {
