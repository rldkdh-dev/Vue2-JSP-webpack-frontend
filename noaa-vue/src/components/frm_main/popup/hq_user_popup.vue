...
export default {
	...
	methods: {
		...
		fnList: function() {
			...
			let data = {
				mapperId: 'comm.selectHqUserList',
			};
		...
		fnFilter:function(){
			let vm = this;
			//일치하는 항목 필터
			vm.userList = vm.userOrgList.filter(data => {
				return data.user_nm.includes(vm.search) || data.dept_nm.includes(vm.search);
			});
		},
		fnUserAdd:function(item) {
			let vm = this;
			//부모창에 선택한 값 전달
			vm.$parent.fnHqMemberAdd(item);
			//팝업 초기화
			vm.$parent.hqUserPop = false;
		},
