...
<modal :pass="{popupTitle: '최초위험성평가 회의록'}">
    <template v-slot:contents>
        ...
        <div id="viewDiv" class="print_page a4 contents">
                        ...
                        <tr v-for="(item,i) in viewModel.mtgUserList" :key="i">
                            ...
                            <td>
                                ...
                                <div class="approval" id="" v-else>
                                    <img class="sign_img" :src="item.sign_img" style="width: 100%;" v-bind:style="item.sign_yn != 'Y' ? 'display: none;' : ''">
                                </div>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div>
                <h3>회의내용 요약</h3>
                <!-- <div class="h300 pre" style="overflow-y:auto">{{ viewModel.data.mtg }}
                    <textarea style="background-color: white;" rows="5" v-model="viewModel.data.mtg" readonly></textarea>
                </div> -->
                <table class="table write">
                    <tr>
                        <td class="pre text-left">{{ viewModel.data.mtg }}</td>
                    </tr>
                </table>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        fnDownlaod: function(){
            let vm = this;
            let fileNm = '최초위험성평가 회의록.pdf';   
            var editorData = $("#viewDiv.a4").html();
            ...
            // .h300 {height: 100%; overflow-y:none;}
            // let divWrite = hiddenDiv.querySelector('.h300');
            // divWrite.style.setProperty('height', '100%');
