<template>
    <div>
            ...
            <div>
                <div class="search_container">
                    <div class="row">
                        ...
                        <input type="text" id="team_nm" placeholder="팀명" title="팀명" v-model="viewModel.search.team_nm">
                ...
                <table class="table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col">일일 작업장 위험성평가표</th>
                            <th scope="col">관리</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(item, i) in viewModel.list" :key=item.wrkshp_risk_evl_sn>
                            ...
                            <td class="text-left" v-if="viewModel.userInfo.bplc_yn != 'Y'">
                                {{ item.bplc_nm }}
                            </td>
                            <td class="text-left"><router-link :to="'/detail/'+item.wrkshp_risk_evl_sn" class="btn-link">{{ item.bplc_team_nm }}</router-link></td>
                            <td>{{ item.job_dt|yyyyMMdd }}</td>
                            <td>
                                <button :class="'btn btn-small btn-blue'" @click="fnViewPopup(item.wrkshp_risk_evl_sn);"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-manage-search"></use></svg>미리보기</button>
                            </td>
                            <td>
                                <router-link :to="'/detail/'+item.wrkshp_risk_evl_sn" class="btn btn-icon"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-content-search"></use></svg></router-link>
                                <template v-if="item.reg_id == viewModel.userInfo.user_id || viewModel.userInfo.ocpt_se_cd == 'MNG'">
                                    <button class="btn btn-icon" @click="fnUpdate(item.wrkshp_risk_evl_sn, item.cnt)"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-edit-contents"></use></svg></button>
                                    <button class="btn btn-icon" @click="fnDelete(item.wrkshp_risk_evl_sn, item.cnt)"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-delete-sweep"></use></svg></button>
                                </template>
                            </td>
                        </tr>
                        <tr v-if="viewModel.list.length == 0"><td :colspan="viewModel.userInfo.bplc_yn != 'Y' ? '6' : '5'">게시물이 없습니다.</td></tr>
                    </tbody>
                </table>
        ...
        <delPopup ref="delPopup" v-if="delPop==true"></delPopup>
    </div>
    </template>
    ...
    <script>
        ...
        import delPopup from "./popup/rgstWpRskEvltFormDelPopup.vue";
            ...
            methods: {
                ...
                fnList:function(paramPageNo){
                    ...
                    let send = {
                        ...
                        job_dt_s: vm.$refs.sdt.value,
                        job_dt_e: vm.$refs.edt.value,
                        bplc_use_yn: vm.viewModel.search.bplc_use_yn,
                        dept_sn: vm.viewModel.userInfo.dept_sn,
                        team_sn: vm.viewModel.userInfo.team_sn,
                    }
                    Axios.post("/selectWpRskEvltFormList.do", send)
                ...
                fnDelete: function(sn, cnt){
                    ...
                    if(cnt > 0){
                        alert("등록된 일일 작업장 위험성평가가 존재하여 삭제가 불가능합니다.");
                        return false;
                    }
