                ...
                <table class="table">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col" :rowspan="2">참석자 서명</th>
                            <th scope="col" :rowspan="2">소공정위험성평가표</th>
                            <th scope="col" :rowspan="2">소공정위험성평가표<br/>업로드</th>
                    ...
                    <tbody>
                        <tr v-for="(item, i) in viewModel.list" :key=item.spcm_risk_evl_sn>
                            ...
                            <td>
                                ...
                                <router-link :to="'/detail/'+item.spcm_risk_evl_sn" v-else-if="item.atdrn_yn == 'Y' && item.sign_yn == 'N'" class="btn btn-small btn-sign"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-signature"></use></svg>서명 전</router-link>
                                <router-link :to="'/detail/'+item.spcm_risk_evl_sn" v-else-if="item.atdrn_yn == 'Y' && item.sign_yn == 'Y'" class="btn btn-small btn-sign complete"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-signature-check"></use></svg>서명 완료</router-link>
                            </td>
                            <td>
                            ...
                            <td>
                                <button class="btn btn-small btn-blue" @click="fnFileUploadPopup(item.spcm_risk_evl_sn);" v-if="item.file_cnt == 0">등록</button>
                                <button class="btn btn-small btn-gray" @click="fnFileUploadPopup(item.spcm_risk_evl_sn);" v-if="item.file_cnt > 0">완료</button>
                            </td>
                            <td>
                                <button class="btn btn-icon" @click="fnVideoUplaodPopup(item.spcm_risk_evl_sn);"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-video-add"></use></svg></button>
                                <button class="btn btn-icon" @click="fnVideoViewPopup(item.spcm_risk_evl_sn);" v-if="item.video_file_cnt > 0"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-video-play"></use></svg></button>      
                            </td>
                            <td>
                                <span class="btn btn-small btn-approval complete" v-if="item.wrtr == viewModel.userInfo.user_id && item.wrtr_sign_yn == 'Y'"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-user-check"></use></svg>결재 완료</span>
                                <span v-else-if="item.wrtr != viewModel.userInfo.user_id && item.wrtr_sign_yn == 'Y'">결재완료</span>
                                <span v-else>진행중</span>
                            </td>
                            <td>
                                <router-link :to="'/detail/'+item.spcm_risk_evl_sn" class="btn btn-small btn-approval" v-if="item.mdtr == viewModel.userInfo.user_id && item.mdtr_sign_yn != 'Y' && item.wrtr_sign_yn == 'Y'"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-approval"></use></svg>결재 요청</router-link>
                            ...
                            <td>
                                <router-link :to="'/detail/'+item.spcm_risk_evl_sn" class="btn btn-small btn-approval" v-if="item.dcsnr == viewModel.userInfo.user_id && item.dcsnr_sign_yn != 'Y' && (item.mdtr == '' || item.mdtr_sign_yn == 'Y') && item.wrtr_sign_yn == 'Y'"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-approval"></use></svg>결재 요청</router-link> 
                                ...
                                <span class="btn btn-small btn-approval" v-else-if="item.dcsnr == viewModel.userInfo.user_id && item.dcsnr_sign_yn != 'Y' && item.mdtr == '' && item.wrtr_sign_yn == 'Y'"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-approval-ongoing"></use></svg>결재요청</span>
                                ...
                                <span v-else-if="item.dcsnr != viewModel.userInfo.user_id && item.dcsnr_sign_yn != 'Y' && item.mdtr != '' && item.wrtr_sign_yn == 'Y'" >진행중</span>
                                <span v-else-if="item.dcsnr != viewModel.userInfo.user_id && item.dcsnr_sign_yn != 'Y' && item.mdtr == '' && item.wrtr_sign_yn == 'Y'" >결재요청</span>
                                <span v-else>진행중</span>
                            </td>
                            <td>
                                ...
                                <template v-if="item.reg_id == viewModel.userInfo.user_id">
                                    <button class="btn btn-icon" @click="fnUpdate(item.spcm_risk_evl_sn, item.dcsnr_sign_yn, item.sign_y_cnt);"><svg class="icon"><use xlink:href="../../img/symbol-defs.svg#icon-edit-contents"></use></svg></button>
                        ...
                        <tr v-if="viewModel.list.length == 0"><td :colspan="viewModel.userInfo.bplc_yn != 'Y' ? '11' : '10'">게시물이 없습니다.</td></tr>
                    </tbody>
                </table>
...
<script>
    ...
    import fileUploadPopup from "./popup/rgstrBsnsAplctFilePopup.vue";
    ...
    export default {
        ...
        methods: {
            ...
            fnUpdate: function(sn, dcsnr_sign_yn, sign_y_cnt){
                let vm = this;

                if(dcsnr_sign_yn == 'Y'){
                    alert("결정자가 결재를 완료하여 수정이 불가합니다.");
                    return false;
                }else if(sign_y_cnt > 0){
                    alert("참석자 서명을 진행하여 수정이 불가합니다.");
                    return false;
                }else{
