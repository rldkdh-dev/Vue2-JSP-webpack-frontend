                        ...
                        <ul>
                            <li v-for="(item, index) in viewModel.userSelectList" :key="item.user_id">
                                <label :for="'bl01_'+index">
                                    <input type="checkbox" title="선택" :value="item" v-model="viewModel.checkList2" :id="'bl01_'+index">{{ item.dept_nm }} {{ item.team_nm }} {{ item.jbgd_nm }} {{ item.user_nm }}
                                </label>
                            </li>
                        </ul>
                    </div>
    ...
    export default {
        ...
        methods: {
            fnInit:function(bplc_sn){
                ...
                vm.viewModel.search.bplc_sn = bplc_sn;
                vm.fnGetInfo();
            },
            fnGetInfo:function(){
                let vm = this;
                ...
                Axios.post("/rgstrBsnsAplctAtdrnInfo.do", data)
                .then(function(response) {
                    vm.viewModel.userList = response.data.userList;                    
                }).then(function(){
            ...
            fnRefresh: function(){
                let vm = this;
                vm.viewModel.search = {user_nm:'', bplc_sn:vm.viewModel.search.bplc_sn};
            },
            fnAdd:function(){
                let vm = this;
