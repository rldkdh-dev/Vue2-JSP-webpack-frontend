...
export default{
    ...
    methods: {
    	...
    	fnInsertFile: function(user_id,bplc_sn) {
            ...
            Axios.post('/mngmt/file/upload.do', frm, {headers: {'Content-Type': 'multipart/form-data'}}).then((response) => {
        ...
        fnAfterDelete: function() {
            ...
            if(vm.viewModel.deletefileList.length != 0){
                for(var i=0; i<vm.viewModel.deletefileList.length; i++){
                    Axios.post("/mngmt/deletefile.do",{file_sn:vm.viewModel.deletefileList[i]}).then(function(){})
                }
            }
        },
