		...
		<div class="contents">
			<div class="box">
				...
				<div>
					...
					<table class="table" id="couponIssueList">
					</table>
					<div id="pager"></div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			fnList: function () {
				...
				Axios.post("/getCouponIssueList.do", senddata).then (function (rs) {
			...
			setGrid: function () {
				...
				let colMod = [
					{name: 'COUPON_NUMBER', index: 'COUPON_NUMBER'},
					{name: 'CD_CP_PYOT', index: 'CD_CP_PYOT'},
					{name: 'NM_CP', index: 'NM_CP'},
					{name: 'NM_CP_DVSN', index: 'NM_CP_DVSN'},
					{name: 'DT_VALID_END', index: 'DT_VALID_END'},
					{name: 'CD_STATUS_COUPON ', index: 'CD_STATUS_COUPON '},
					{name: 'MEMBER_NAME ', index: 'MEMBER_NAME'},
					{name: 'DT_USE_COUPON', index: 'DT_USE_COUPON'}
				];
				
				$("#couponIssueList").jqGrid({
					dataType : 'local',
					data : vm.viewModel.list,
					colNames : ['쿠폰번호', '발행처', '쿠폰명','쿠폰종류','유효기간','상태','회원명','사용일자'],
					colModel : colMod,
					rowNum : vm.viewModel.pager.rowSize,
					pager : '#pager',
					sortname : 'COUPON_NUMBER', //쿠폰번호
					viewrecords: true,
					sortable: true,
					sortorder : 'DESC',
					height : 'auto',
					...
					onSelectRow: function (id) {
						var rowData = $('#couponIssueList').getRowData(id);
						vm.$router.push({path: '/detail', name:'issuedetail', query : {key : rowData.SEQ_CP_NUM}});
					}
				})
