        ...
        <div class="box">
            ...
            <div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(item, i) in viewModel.list.filter(data => data.EVENT_NM !== '시즌총계')" :key="i">
                        <td>{{ item.ROWNUM }}</td>
                        <td>{{ item.EVENT_NM }}</td>
                        <td>{{ item.GAME_DT }}</td>
                        <td>{{ item.GWEEK }}</td>
                        ...
                        <td>{{ item.PAYMENT_Y }}</td>
                        <td>{{ item.TICKET_CNT_H }}</td>
                        <td>{{ item.PAYMENT_H }}</td>
                    ...
                    <tfoot>
                        <tr v-for="(item, i) in viewModel.list.filter(data => data.EVENT_NM === '시즌총계')" :key="'tfoot-' + i">
                        <td colspan="5">시즌총계</td>
                        <td>{{ item.TICKET_CNT_Y }}</td>
                        <td>{{ item.PAYMENT_Y }}</td>
                        <td>{{ item.TICKET_CNT_H }}</td>
                        <td>{{ item.PAYMENT_H }}</td>
...
export default {
    ...
    methods: {
        fnList: function () {
            ...
            Axios.post("/getGameList.do", send).then (function (rs) {
        ...
        fnExcelDownload: function () {
                ...
                Axios.post("/getGameList.do", send)
                    .then(function (response) {
                        excelList = response.data.list;
                        // console.log(excelList);
                        for (const ex of excelList) {
                            
                            let excel = {No:'', 상세이벤트명:'', 날짜:'', 요일:'', 시간:'', 예매매수:'', 예매금액:'', 현장매수:'', 현장금액:'', 합계매수:'', 합계금액:'', 평균예매율:'', 평균단가:'',};

                            excel.No = ex.ROWNUM;
                            excel.상세이벤트명 = ex.EVENT_NM;
                            excel.날짜 = ex.GAME_DT;
                            excel.요일 = ex.GWEEK;
                            excel.시간 = ex.GTIME;
                            excel.예매매수 = ex.TICKET_CNT_Y;
                            excel.예매금액 = ex.PAYMENT_Y;
                            excel.현장매수 = ex.TICKET_CNT_H;
                            excel.현장금액 = ex.PAYMENT_H;
                            excel.합계매수 = ex.TOTAL_CNT;
                            excel.합계금액 = ex.TOTAL_AMT;
                            excel.평균예매율 = ex.AVR_CNT;
                            excel.평균단가 = ex.AVR_AMT;
                        ...
                        try {
                            ...
                            Xlsx.utils.book_append_sheet(workBook, workSheet, '경기별 현황');
                            Xlsx.writeFile(workBook, '경기별 현황.xlsx');
                            vm.excelDownload = [];
                        } catch (er) {
